...
const Home = ({ setRoute, setUser }) => {
  ...
  return (
    <div className="w-full bg-white relative overflow-hidden flex flex-col items-center">
      ...
      {/* Main Hero Section */}
      <section className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 lg:pt-24 pb-12 w-full text-center flex flex-col items-center border-b border-transparent">
        
        {/* Release Pill */}
        <div className="mb-6 inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-brand-200 bg-brand-50 text-brand-700 text-xs sm:text-sm font-medium hover:bg-brand-100 transition-colors cursor-pointer shadow-sm animate-fade-in-up">
          <Sparkles size={15} className="text-brand-500" />
          <span>Introducing AcadNexus Intelligence Engine</span>
          <ArrowRight size={13} className="ml-1 text-brand-500" />
        </div>

        {/* Headlines */}
        <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-slate-900 tracking-tight leading-[1.15] mb-6 max-w-4xl animate-fade-in-up" style={{animationDelay: '0.1s'}}>
        ...
        <p className="text-base sm:text-lg text-slate-600 mb-8 max-w-2xl leading-relaxed animate-fade-in-up" style={{animationDelay: '0.2s'}}>
