...
const StudyPlanner = ({ user }) => {
  ...
  return (
    <div className="relative min-h-[calc(100vh-5rem)] max-w-5xl mx-auto space-y-10 animate-fade-in-up pb-12 pt-4 z-10">
      ...
      <div className="relative rounded-2xl sm:rounded-[32px] overflow-hidden shadow-[0_8px_40px_rgb(0,0,0,0.08)] mb-6 sm:mb-10 group">
         ...
         <div className="relative z-10 p-5 sm:p-10 lg:p-14 max-w-3xl">
            <div className="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-white/90 backdrop-blur-md text-brand-600 text-xs sm:text-sm font-bold mb-4 sm:mb-6 shadow-sm border border-brand-100">
              <Sparkles size={16} /> AI Adaptive Planner
            </div>
            <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black text-slate-900 tracking-tight leading-[1.1] mb-3 sm:mb-6">
            ...
            <p className="text-sm sm:text-lg text-slate-600 font-medium leading-relaxed backdrop-blur-sm bg-white/40 p-2 sm:p-3 rounded-xl max-w-xl">
      ...
      {step === 'setup' && (
        <div className="space-y-8 animate-fade-in-up">
           <PremiumCard className="p-5 sm:p-8 md:p-10">
             <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8 border-b border-slate-100 pb-4 sm:pb-6">
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-brand-100 rounded-xl sm:rounded-2xl flex items-center justify-center text-brand-600 shrink-0">
                   <BookOpen size={22} />
                </div>
                <div>
                   <h2 className="text-xl sm:text-2xl font-black text-slate-800">Your Subjects</h2>
                   <p className="text-xs sm:text-sm text-slate-500 font-medium">Add subjects and drop images of your syllabus to dynamically set goals.</p>
                </div>
             </div>

              <div className="space-y-5">
                {subjects.map((subj) => (
                  <div key={subj.id} className="group flex flex-col md:flex-row items-stretch md:items-center gap-3 sm:gap-4 p-4 rounded-2xl bg-white border border-slate-100 hover:border-brand-200 hover:shadow-md hover:bg-slate-50 transition-all shadow-sm">
                     <div className="flex-1 w-full">
                       ...
                       <input 
                         ...
                         className="w-full bg-white px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all outline-none font-bold text-slate-800 placeholder-slate-400 shadow-sm text-sm" 
                       />
                     </div>
                     ...
                     <div className="w-full md:w-48">
                       ...
                       <select 
                         ...
                         className="w-full bg-white px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-brand-500 outline-none font-bold text-slate-800 appearance-none cursor-pointer shadow-sm text-sm"
                       >
                     ...
                     <div className="w-full md:w-auto relative flex gap-2">
                        ...
                        <button 
                          ...
                          className={`flex-1 md:w-auto px-4 py-2.5 sm:py-3 flex items-center justify-center gap-2 rounded-xl border font-bold transition-all shadow-sm text-xs sm:text-sm ${
                        ...
                        >
                          {subj.syllabusStatus === 'success' ? <Check size={16} /> : 
                           subj.syllabusStatus === 'extracting' ? <BrainCircuit size={16} className="animate-spin" /> : 
                           <Upload size={16} />}
                        ...
                        {subjects.length > 1 && (
                          <button onClick={() => removeSubject(subj.id)} className="p-2.5 sm:p-3 text-slate-400 hover:text-red-600 hover:bg-red-500/20 border border-slate-200 rounded-xl transition-all flex justify-center items-center">
                            <Trash2 size={18} />
                          </button>
                        )}
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-8">
              <PremiumCard className="p-5 sm:p-8 md:p-10 relative overflow-hidden group">
                 ...
                 <h2 className="text-xl sm:text-2xl font-black text-slate-800 mb-1 sm:mb-2">Available Time</h2>
                 <p className="text-xs sm:text-sm text-slate-500 font-medium mb-4 sm:mb-6">How many hours can you dedicate today?</p>
                 
                 <div className="flex items-center gap-4 sm:gap-6 bg-slate-50 p-4 sm:p-6 rounded-2xl border border-slate-100 shadow-inner">
                    <button onClick={() => setHours(Math.max(1, hours - 1))} className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-white shadow-sm border border-slate-200 text-xl sm:text-2xl font-black text-slate-700 hover:bg-slate-100 hover:text-brand-700 transition-all">-</button>
                    <div className="flex-1 text-center">
                       <span className="text-3xl sm:text-5xl font-black text-slate-900">{hours}</span>
                       <span className="text-sm sm:text-lg font-bold text-slate-600 ml-1 sm:ml-2">hrs</span>
                    </div>
                    <button onClick={() => setHours(Math.min(12, hours + 1))} className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-white shadow-sm border border-slate-200 text-xl sm:text-2xl font-black text-slate-700 hover:bg-slate-100 hover:text-brand-700 transition-all">+</button>
                 </div>
              </PremiumCard>

              <div className="flex flex-col justify-end">
                 <button onClick={generatePlan} className="w-full py-5 sm:py-8 rounded-2xl sm:rounded-[32px] bg-slate-900 relative overflow-hidden group shadow-2xl hover:shadow-[0_20px_40px_rgba(0,0,0,0.2)] hover:-translate-y-1 transition-all duration-300">
                    ...
                    <div className="relative z-10 flex items-center justify-center gap-2 sm:gap-3 text-white">
                      ...
                      <span className="text-lg sm:text-2xl font-black tracking-tight">Synthesize Plan</span>
                    </div>
                 </button>
              </div>
           </div>
